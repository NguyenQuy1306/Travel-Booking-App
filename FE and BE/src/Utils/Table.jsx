...
function GlobalFilter({
  ...
  return (
    <label className="flex gap-x-2 items-baseline">
      ...
      <input
        type="text"
        className="rounded-md border-gray-500 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
        value={value || ""}
        onChange={e => {
...
function Table({ columns, data }) {
  ...
  const handleAction = (row,cell) => {
    if (cell.column.id === "delete") {
      ...
      ><img src="/close.svg" className='h-7'/></button>
    }
    ...
    ><img src="/paper-plane.svg" className='h-7'/></button>
      ...
      return <button><img src="/add-remove.svg" className='h-5'/></button>
    }
    else if (cell.column.id === "Modify"){
      return <button><img src="/fix.svg" className='h-5'/></button>
    }
          ...
          <div className="py-2 align-middle inline-block min-w-full sm:px-6 lg:px-8">
            <div className="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
              <table {...getTableProps()} className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50 text-center justify-center">
                  {headerGroups.map(headerGroup => (
                    <tr {...headerGroup.getHeaderGroupProps()}>
                      {headerGroup.headers.map(column => (
                        ...
                        >
                          <div className="flex items-center justify-between font-bold text-xs md:text-sm text-center">
                            {/* show header title of each column */}
                            <div className='w-full justify-center flex'></div>
                            <div>
                            <div className='w-full justify-center flex'>{column.name}</div>
